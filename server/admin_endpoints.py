...
All routes enforce server-side RBAC.
"""
...
def register_admin_endpoints(app: FastAPI, settings: Settings) -> None:
    """Register all admin endpoints with RBAC enforcement."""
    ...
    async def get_performance_status(
        ...
        """Get current performance optimization status (admin read)."""
        try:
    ...
    async def run_performance_audit(
        ...
        """Run a performance audit (admin manage)."""
        try:
    ...
    async def trigger_optimization(
        ...
        """Trigger performance optimization (admin manage)."""
        try:
    ...
    async def get_validation_config(
        ...
        """Get current validation configuration (admin read)."""
        try:
    ...
    async def reload_validation_config(
        current_user: Dict[str, Any] = Depends(require_permission(Permission.ADMIN_SYSTEM)),
    ):
        """Reload validation configuration from environment (admin system)."""
        try:
    ...
    async def update_validation_patterns(
        request: dict,
        current_user: Dict[str, Any] = Depends(require_permission(Permission.ADMIN_SYSTEM)),
    ):
        """Update validation patterns without code changes (admin system)."""
        try:
            import ai_karen_engine.server.middleware as middleware_module
            validation_config = getattr(middleware_module, '_validation_config', None)
    ...
    async def get_validation_status(
        ...
        """Get validation system status and metrics (admin read)."""
        try:
            import ai_karen_engine.server.middleware as middleware_module
            validation_config = getattr(middleware_module, '_validation_config', None)
