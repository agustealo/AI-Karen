"""COG-EVAL-1: Memory security / poisoning benchmark."""
...
CASES = scenarios_for(ScenarioKind.MEMORY_POISONING)


@pytest.mark.cognitive
@pytest.mark.memory_security
@pytest.mark.parametrize("scenario", CASES, ids=[s.scenario_id for s in CASES])
def test_memory_security(scenario):
