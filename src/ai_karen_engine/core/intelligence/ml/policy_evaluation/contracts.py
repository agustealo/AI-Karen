"""Neutral contracts for counterfactual policy evaluation and safe promotion.

These types describe logged decisions, propensity distributions, utility, and
off-policy estimates. They are evidence contracts only. They do not authorize
or execute actions and they do not own runtime routing policy.
"""
...
def validate_probability_distribution(
    ...
    for action in eligible_actions:
        ...
        if np.isnan(probability) or np.isinf(probability) or probability < 0.0:
            return f"invalid_probability:{action}"
    total = sum(probabilities.get(action, 0.0) for action in eligible_actions)
