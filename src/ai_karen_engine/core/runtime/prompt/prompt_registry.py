...
class PromptRegistry:
    ...
    def enforce_token_budget(self, request: PromptAssemblyRequest, estimate: TokenEstimate) -> Dict[str, Any]:
        """Enforce token budget and return enforcement result."""
        
        if estimate.total_tokens <= request.token_budget:
            return {
                "budget_ok": True,
                "tokens_used": estimate.total_tokens,
                "tokens_remaining": request.token_budget - estimate.total_tokens,
                "truncation_events": [],
            }
        
        # Budget exceeded - calculate truncation strategy
        truncation_events = []
        ...
        for field_name, field_type in truncation_strategies:
            ...
            if field_name == "messages":
                # Truncate from the end of the conversation
                original_count = len(field_value)
                tokens_per_message = estimate.message_tokens / original_count if original_count > 0 else 0
                
                max_messages = int(remaining_tokens / tokens_per_message) if tokens_per_message > 0 else 0
                if max_messages < original_count:
                    truncated_messages = field_value[:max_messages]
                    setattr(request, field_name, truncated_messages)
                    
                    removed_count = original_count - max_messages
                    truncation_events.append(PromptTruncationEvent(
                        section="messages",
                        reason="budget_exceeded",
                        original_tokens=estimate.message_tokens,
                        remaining_tokens=max_messages * tokens_per_message,
                        items_removed=removed_count,
                    ))
                    
                    remaining_tokens -= max_messages * tokens_per_message
            
            elif field_name in ["memory_items", "tool_contracts"]:
                # Truncate from the end of the list
                original_count = len(field_value)
                tokens_per_item = estimate.to_dict()[f"{field_type}_tokens"] / original_count if original_count > 0 else 0
                
                max_items = int(remaining_tokens / tokens_per_item) if tokens_per_item > 0 else 0
                if max_items < original_count:
                    truncated_items = field_value[:max_items]
                    setattr(request, field_name, truncated_items)
                    
                    removed_count = original_count - max_items
                    truncation_events.append(PromptTruncationEvent(
                        section=field_type,
                        reason="budget_exceeded",
                        original_tokens=estimate.to_dict()[f"{field_type}_tokens"],
                        remaining_tokens=max_items * tokens_per_item,
                        items_removed=removed_count,
                    ))
                    
                    remaining_tokens -= max_items * tokens_per_item
            
            elif field_name == "system_instructions":
                # Truncate system instructions
                original_text = field_value
                tokens_available = remaining_tokens
                
                if tokens_available < len(original_text.split()):
                    # Simple truncation - take first N words
                    words = original_text.split()
                    max_words = int(tokens_available)
                    truncated_text = " ".join(words[:max_words])
                    setattr(request, field_name, truncated_text)
                    
                    truncation_events.append(PromptTruncationEvent(
                        section="system_instructions",
                        reason="budget_exceeded",
                        original_tokens=len(words),
                        remaining_tokens=max_words,
                        items_removed=1,
                    ))
        ...
        return {
            "budget_ok": False,
            "tokens_used": request.token_budget,
            "tokens_remaining": 0,
            "truncation_events": [event.__dict__ for event in truncation_events],
        }
