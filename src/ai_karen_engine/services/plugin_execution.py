...
class PluginExecutionEngine:
    ...
    async def _validate_execution_authorization(
        ...
        """Validate that plugin execution is authorized by a matching runtime plan."""
        if plan is None:
            raise ValueError(
                f"Plugin '{context.plugin_id}' requires an AuthorizedExecutionPlan."
            )
        if not context.policy_decision_id:
            raise ValueError(
                f"Plugin '{context.plugin_id}' requires a policy_decision_id for execution."
            )
        if context.policy_decision_id != plan.policy_decision_id:
            raise ValueError(
                f"Plugin '{context.plugin_id}' policy decision does not match the authorized plan."
            )
        if not plan.matches_execution_scope(
            ...
            raise ValueError(
                f"Plugin '{context.plugin_id}' execution scope does not match the authorized plan."
            )
        if context.plugin_id not in plan.allowed_plugins:
            raise ValueError(
                f"Plugin '{context.plugin_id}' is not present in AuthorizedExecutionPlan.allowed_plugins."
            )
        if not await ActionExecutionGate.authorize(plan, context.plugin_id):
            raise ValueError(
                f"Plugin '{context.plugin_id}' denied by ActionExecutionGate."
            )
    ...
    async def execute_plugin(
        ...
        """Execute a plugin with the specified parameters through runtime authorization."""
        ...
        try:
            if plan is None:
                raise ValueError(
                    f"Plugin '{request.plugin_name}' requires an AuthorizedExecutionPlan."
                )
            ...
            await self._validate_execution_authorization(
                execution_context, manifest, plan=plan
            )
